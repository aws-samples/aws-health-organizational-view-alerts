...
def send_webhook(updatedOn, strStartTime, strEndTime, event, awsRegion, decodedWebHook, healthUpdates, affectedAccounts, affectedEntities):
    ...
    teams_message = {
                    ...
                    "themeColor": "FF0000",
                    "summary": "AWS Health Org View Alert",
                    "sections": [
                        {
                            "activityTitle": "&#x1F6A8; AWS Health Org View Alert &#x1F6A8;",
                            "facts": [
                                { "name": "Account(s)", "value": affectedAccounts},
                                { "name": "Resource(s)", "value": affectedEntities},
                                { "name": "Service", "value": str(event['service'])},
                                { "name": "Region", "value": str(event['region'])},
                                { "name": "Start Time (UTC)", "value": strStartTime},
                                { "name": "End Time (UTC)", "value": strEndTime},
                                { "name": "Posted Time (UTC)", "value": updatedOn},
                                { "name": "Status", "value": str(event['statusCode'])},
                                { "name": "Updates", "value": str(healthUpdates)}
                                ],
                        }
        ]
    }
    req = Request(decodedWebHook, data=json.dumps(teams_message).encode("utf-8"), headers={"content-type": "application/json"})
    try:
      response = urlopen(req)
      response.read()
      print("Message sent to teams: ", json.dumps(teams_message))
